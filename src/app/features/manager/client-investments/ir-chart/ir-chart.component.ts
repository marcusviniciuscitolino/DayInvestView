...
import { IrValuesDialogComponent } from './ir-values-dialog/ir-values-dialog.component';

@Component({
  selector: 'app-ir-chart',
  templateUrl: './ir-chart.component.html',
  styleUrls: ['./ir-chart.component.scss']
})
export class IrChartComponent implements OnInit, OnChanges {
  ...
  chartOptions: ChartConfiguration<'bar'>['options'] = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Evolução de IR (Imposto de Renda) ao Longo do Tempo'
      },
      tooltip: {
        callbacks: {
          label: (context: any) => {
  ...
  private updateChart(): void {
    ...
    const irMovements = this.movements.filter(m => m.movementType === 'IR');
    
    if (irMovements.length === 0) {
      this.chartData = {
        labels: [],
        datasets: []
      };
    ...
    // Agrupar por ativo (só NTN-B tem IR)
    const assets = [...new Set(irMovements.map(m => m.assetName))];
    
    // Agrupar por data e ativo
    const dataMap = new Map<string, Map<string, number>>();
    
    irMovements.forEach(movement => {
      ...
      // Usa quotaValue (Valor Cota Moviment) para o IR
      const irValue = movement.quotaValue || 0;
      assetMap.set(movement.assetName, currentValue + irValue);
    });
    ...
    const datasets = assets.map((asset, index) => {
      const colors = [
        'rgba(244, 67, 54, 0.8)',   // Vermelho (IR)
  ...
  openValuesDialog(): void {
    ...
    this.dialog.open(IrValuesDialogComponent, {
      width: '700px',
      maxWidth: '90vw',
      data: {
